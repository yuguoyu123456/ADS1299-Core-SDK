# Contact and Collaboration

ADS1299-Core-SDK is an open engineering and learning project for the ADS1299 ecosystem.

## Maintainer

**Biomedical Engineering / Medical PhD researcher**

I welcome contact from students, researchers, engineers, educators, hardware developers, BCI teams and companies working with ADS1299, EEG, electrophysiology, embedded systems or neurotechnology.

## Email

- Gmail: **yuguoyu520@gmail.com**
- Email: **yuguoyu123456@126.com**

You are welcome to email me about:

- ADS1299 hardware bring-up and debugging
- MCU / FPGA platform ports
- EEG acquisition and signal processing
- BCI experiments and algorithms
- research collaboration
- teaching and educational use
- hardware validation reports
- bug reports and feature requests
- commercial integration discussions
- documentation improvements

## Public technical questions

For questions that may help other users, please prefer a GitHub Issue or Discussion when available. Public problem-solving makes the repository more useful to the whole ADS1299 community.

## Contributions

Pull requests are welcome. Please read [`CONTRIBUTING.md`](CONTRIBUTING.md) before submitting changes.

The project values reproducibility, clear documentation, real buildable projects and evidence-based validation.
