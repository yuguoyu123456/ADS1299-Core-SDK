# RK3568 ADS1299 Linux Quick Start

Validation status: **TEMPLATE / HOST-COMPILE TARGET** until the dedicated CI for this leaf passes. No physical RK3568 board or ADS1299 hardware is claimed as BOARD-VERIFIED.

## 1. Board/BSP prerequisite

Use an RK3568 Linux board whose BSP exposes the selected SPI controller as `/dev/spidevX.Y` and the required GPIO bank as `/dev/gpiochipN`. The exact carrier-board header pins and GPIO line offsets vary by board/BSP, so this folder intentionally does not invent pin numbers.
...
```sh
cd firmware/linux_compute/Rockchip/RK3568
make clean check
```
...
```text
RK3568 Linux userspace compile OK: build/rk3568_ads1299_probe
...
Not yet candidate-complete: named internal-test, input-short and 250-SPS EEG runnable flows; bounded DRDY streaming; multi-device example; and a physically verified RK3568 reference-board wiring table. Those should be added without moving ADS1299 register logic into this platform folder.
