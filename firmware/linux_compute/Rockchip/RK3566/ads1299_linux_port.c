...
static void fds_init(rk3566_ads1299_linux_t *c){c->spi_fd=c->cs_fd=c->reset_fd=c->pwdn_fd=c->start_fd=c->drdy_fd=-1;}
...
static int spi_cb(void *u,const uint8_t *tx,uint8_t *rx,size_t n){rk3566_ads1299_linux_t *c=u; struct spi_ioc_transfer t; if(!c||c->spi_fd<0||!n)return -EINVAL; memset(&t,0,sizeof(t)); t.tx_buf=(uintptr_t)tx; t.rx_buf=(uintptr_t)rx; t.len=(uint32_t)n; t.speed_hz=c->spi_hz; t.bits_per_word=8u; return ioctl(c->spi_fd,SPI_IOC_MESSAGE(1),&t)<0?-errno:0;}
static void cs_cb(void*u,int v){rk3566_ads1299_linux_t*c=u;if(c)(void)line_set(c->cs_fd,v);} static void rst_cb(void*u,int v){rk3566_ads1299_linux_t*c=u;if(c&&c->reset_fd>=0)(void)line_set(c->reset_fd,v);} static void pwdn_cb(void*u,int v){rk3566_ads1299_linux_t*c=u;if(c&&c->pwdn_fd>=0)(void)line_set(c->pwdn_fd,v);} static void start_cb(void*u,int v){rk3566_ads1299_linux_t*c=u;if(c&&c->start_fd>=0)(void)line_set(c->start_fd,v);} static int drdy_cb(void*u){rk3566_ads1299_linux_t*c=u;return c?line_get(c->drdy_fd):-EINVAL;}
