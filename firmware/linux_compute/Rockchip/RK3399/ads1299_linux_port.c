...
static void init_fds(rk3399_ads1299_linux_t *ctx) {
    ctx->spi_fd = -1;
    ctx->cs_fd = -1;
    ctx->reset_fd = -1;
    ctx->pwdn_fd = -1;
...
static int request_line(int chip_fd, unsigned int offset, unsigned long flags,
    ...
    strncpy(request.consumer_label, label, sizeof(request.consumer_label) - 1u);
    if (ioctl(chip_fd, GPIO_GET_LINEHANDLE_IOCTL, &request) < 0) return -errno;
    return request.fd;
}
...
static int set_line(int fd, int level) {
    ...
    if (ioctl(fd, GPIOHANDLE_SET_LINE_VALUES_IOCTL, &data) < 0) return -errno;
    return 0;
}
...
static int get_line(int fd) {
    ...
    struct gpiohandle_data data;
    memset(&data, 0, sizeof(data));
    if (ioctl(fd, GPIOHANDLE_GET_LINE_VALUES_IOCTL, &data) < 0) return -errno;
    return data.values[0] ? 1 : 0;
}

static int spi_transfer_cb(void *user, const uint8_t *tx, uint8_t *rx, size_t len) {
    rk3399_ads1299_linux_t *ctx = (rk3399_ads1299_linux_t *)user;
    ...
    return ioctl(ctx->spi_fd, SPI_IOC_MESSAGE(1), &transfer) < 0 ? -errno : 0;
}
...
static void delay_us_cb(void *user, uint32_t us) {
    (void)user;
    struct timespec req = {(time_t)(us / 1000000u), (long)((us % 1000000u) * 1000u)};
    while (nanosleep(&req, &req) < 0 && errno == EINTR) {}
}

int rk3399_ads1299_linux_open(rk3399_ads1299_linux_t *ctx,
                              const rk3399_ads1299_linux_config_t *config) {
    if (!ctx || !config || !config->spidev_path || !config->gpiochip_path ||
        config->spi_hz == 0u || config->cs_line == UINT_MAX) return -EINVAL;
    ...
    if (ioctl(ctx->spi_fd, SPI_IOC_WR_MODE, &mode) < 0 ||
        ...
        int saved = errno;
        rk3399_ads1299_linux_close(ctx);
        return -saved;
    }

    int chip_fd = open(config->gpiochip_path, O_RDONLY | O_CLOEXEC);
    if (chip_fd < 0) {
        int saved = errno;
        rk3399_ads1299_linux_close(ctx);
        return -saved;
    ...
    ctx->cs_fd = request_line(chip_fd, config->cs_line, GPIOHANDLE_REQUEST_OUTPUT, 1, "ads1299-cs");
    if (ctx->cs_fd < 0) {
        int saved = -ctx->cs_fd;
        close(chip_fd);
        rk3399_ads1299_linux_close(ctx);
    ...
    ctx->reset_fd = request_optional_line(chip_fd, config->reset_line, GPIOHANDLE_REQUEST_OUTPUT, 1, "ads1299-reset");
    ctx->pwdn_fd = request_optional_line(chip_fd, config->pwdn_line, GPIOHANDLE_REQUEST_OUTPUT, 1, "ads1299-pwdn");
    ctx->start_fd = request_optional_line(chip_fd, config->start_line, GPIOHANDLE_REQUEST_OUTPUT, 0, "ads1299-start");
    ctx->drdy_fd = request_optional_line(chip_fd, config->drdy_line, GPIOHANDLE_REQUEST_INPUT, 0, "ads1299-drdy");
    close(chip_fd);
...
void rk3399_ads1299_linux_close(rk3399_ads1299_linux_t *ctx) {
    if (!ctx) return;
...
int rk3399_ads1299_linux_bind(rk3399_ads1299_linux_t *ctx, ads1299_port_t *port) {
