# Nuvoton NuMicro + ADS1299 Direct-Use Guide

This guide is additive and preserves every existing model-specific `ads1299_port` file.

Configure the selected NuMicro SPI peripheral as master, 8-bit, MSB-first, ADS1299 mode 1 (CPOL=0, CPHA=1). Start around 2-4 MHz for bring-up. Configure CS, RESET, PWDN and START as outputs and DRDY as a falling-edge interrupt input. Keep actual pins board-specific.
...
9. Push the frame into a fixed-size ring buffer for USB/UART/Ethernet/other transport processing.
...
For 8 x ADS1299 / 64 channels, use multiple SPI/DMA lanes when practical. For strict synchronization and low inter-device skew, use `firmware/fpga/common` as the deterministic acquisition front end and let NuMicro handle control and transport.
