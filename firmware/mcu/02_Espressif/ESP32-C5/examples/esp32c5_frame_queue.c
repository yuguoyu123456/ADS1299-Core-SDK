                                       ...
                                       uint32_t sequence)
{
    ...
    if (q->count >= ESP32C5_ADS1299_FRAME_QUEUE_CAPACITY) {
        ++q->dropped;
        return false;
    }
    ...
    q->head = (q->head + 1u) % ESP32C5_ADS1299_FRAME_QUEUE_CAPACITY;
    ++q->count;
                                      ...
                                      esp32c5_ads1299_frame_record_t *out)
{
    ...
    *out = q->items[q->tail];
    q->tail = (q->tail + 1u) % ESP32C5_ADS1299_FRAME_QUEUE_CAPACITY;
    --q->count;
    return true;
}
