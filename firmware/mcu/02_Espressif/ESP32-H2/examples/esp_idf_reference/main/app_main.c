...
static const char *TAG = "ads1299-h2";
...
static esp32h2_ads1299_frame_queue_t g_queue;
...
static void acquisition_task(void *arg)
{
    ...
    for (;;) {
        if (g_port.drdy_read(g_port.user) == 0) {
            ...
            (void)esp32h2_ads1299_frame_queue_push(&g_queue, &frame, ts, seq);
...
{
    (void)arg;
    esp32h2_ads1299_frame_record_t rec;
    ...
    for (;;) {
        ...
        size_t high;
        portENTER_CRITICAL(&g_queue_lock);
        have = esp32h2_ads1299_frame_queue_pop(&g_queue, &rec);
        dropped = esp32h2_ads1299_frame_queue_dropped(&g_queue);
        high = esp32h2_ads1299_frame_queue_high_watermark(&g_queue);
        portEXIT_CRITICAL(&g_queue_lock);

        if (dropped != last_dropped) {
            ESP_LOGW(TAG, "queue overflow dropped=%" PRIu32 " high=%u/%u",
                     dropped, (unsigned)high, (unsigned)ESP32H2_ADS1299_FRAME_QUEUE_CAPACITY);
...
void app_main(void)
{
    ...
    esp32h2_ads1299_frame_queue_init(&g_queue);
    g_sequence = 0u;
