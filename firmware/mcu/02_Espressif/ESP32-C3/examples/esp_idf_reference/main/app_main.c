...
static const char *TAG = "ads1299-c3";
...
{
    (void)arg;
    esp32c3_ads1299_frame_record_t rec;
    ...
    for (;;) {
        ...
        size_t high;
        portENTER_CRITICAL(&g_queue_lock);
        have = esp32c3_ads1299_frame_queue_pop(&g_queue, &rec);
        dropped = esp32c3_ads1299_frame_queue_dropped(&g_queue);
        high = esp32c3_ads1299_frame_queue_high_watermark(&g_queue);
        portEXIT_CRITICAL(&g_queue_lock);

        if (dropped != last_dropped) {
            ESP_LOGW(TAG, "queue overflow dropped=%" PRIu32 " high=%u/%u",
                     dropped, (unsigned)high, (unsigned)ESP32C3_ADS1299_FRAME_QUEUE_CAPACITY);
