...
static ads1299_c6_context_t ctx;
...
static int prepare(void)
{
    ...
    return ads1299_c6_make_hal(&ctx,&hal);
}
int test_main(void)
{
    uint8_t tx[65],rx[65];
    ...
    CHECK(prepare()==0); ctx.pins[4]=ctx.pins[0]; CHECK(ads1299_c6_make_hal(&ctx,&hal)!=0);
    CHECK(prepare()==0); ctx.timeout_ticks=portMAX_DELAY; CHECK(ads1299_c6_make_hal(&ctx,&hal)!=0);
    CHECK(prepare()==0); ctx.pins[4]=100; CHECK(ads1299_c6_make_hal(&ctx,&hal)!=0);
    CHECK(prepare()==0); ctx.pins[4]=-1; CHECK(ads1299_c6_make_hal(&ctx,&hal)!=0);
    return 0;
}
