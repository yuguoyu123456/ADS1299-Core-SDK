...
#include "stm32f091_example_platform.h"
...
static int wait_and_read(ads1299_t *dev, ads1299_frame_t *frame)
{
    ...
    if (st != ADS1299_OK) {
        stm32f091_example_log("ERR DRDY timeout/check DRDY wiring\r\n");
        return -1;
    }
    if (ads1299_read_frame_continuous(dev, frame) != ADS1299_OK) {
        stm32f091_example_log("ERR frame read/check SPI and CS\r\n");
        return -2;
    }
    return 0;
}
...
static int probe_device(ads1299_t *dev)
{
    ...
    if (ads1299_hardware_reset(dev) != ADS1299_OK) {
        stm32f091_example_log("ERR RESET/check RESET PWDN power\r\n");
        return -1;
    }
    if (ads1299_sdatac(dev) != ADS1299_OK) {
        stm32f091_example_log("ERR SDATAC/check SPI Mode 1 CS\r\n");
        return -2;
    }
    if (ads1299_read_device_id(dev, &id) != ADS1299_OK || !id.is_ads1299_family) {
        stm32f091_example_log("ERR ID/not ADS1299 family; check SPI Mode 1 power clock\r\n");
        return -3;
    }
    if (id.channel_count == 8u) stm32f091_example_log("OK ID ADS1299-8\r\n");
    else if (id.channel_count == 6u) stm32f091_example_log("OK ID ADS1299-6\r\n");
    else if (id.channel_count == 4u) stm32f091_example_log("OK ID ADS1299-4\r\n");
    else return -4;
    return 0;
}
...
{
    stm32f091_example_log("RUN internal-test\r\n");
    ...
    stm32f091_example_log("OK internal-test frames\r\n");
    return 0;
}

static int run_input_short(ads1299_t *dev)
{
    stm32f091_example_log("RUN input-short\r\n");
    if (ads1299_sdatac(dev) != ADS1299_OK) return -1;
    ...
    if (stop_continuous(dev) != 0) return -5;
    stm32f091_example_log("OK input-short frames\r\n");
    return 0;
}
...
static int configure_eeg_250sps(ads1299_t *dev)
{
    ...
    stm32f091_example_log("RUN EEG 250 SPS config\r\n");
    if (ads1299_sdatac(dev) != ADS1299_OK) return -1;
    ...
    stm32f091_example_log("OK EEG 250 SPS configured\r\n");
    return 0;
}
...
static int stream_packets(ads1299_t *dev, uint32_t stream_frames)
{
    ...
    stm32f091_example_log("RUN stream canonical 49-byte packets\r\n");
    ...
    while (stream_frames == 0u || sequence < stream_frames) {
        ...
        packet_len = ads1299_packet_encode(packet, sizeof(packet), 0u, sequence,
                                           stm32f091_example_timestamp_us(), frame.status, frame.channel);
        if (packet_len != ADS1299_PACKET_SIZE) {
            (void)stop_continuous(dev);
            return -3;
        }
        if (stm32f091_example_write(packet, packet_len) != 0) {
            stm32f091_example_log("ERR stream transport/check UART config\r\n");
            (void)stop_continuous(dev);
            return -4;
        }
        ++sequence;
...
int stm32f091_ads1299_beginner_demo(uint32_t stream_frames)
{
    ...
    stm32f091_example_log("ADS1299 STM32F091 beginner demo\r\n");
    if (stm32f091_ads1299_make_hal(&hal) != 0) return -10;
    ...
    stm32f091_example_log("OK beginner demo complete\r\n");
    return 0;
}
