...
int stm32f091_ads1299_make_hal(ads1299_platform_hal_t *hal)
{
...
{
    return stm32f091_ads1299_make_hal(hal);
}

int stm32f091_example_write(const uint8_t *data, size_t len)
{
...
void stm32f091_example_log(const char *text)
{
    if (text == NULL) return;
#if ADS1299_BOARD_USE_UART_STREAM
    (void)stm32f091_example_write((const uint8_t *)text, strlen(text));
#else
    (void)text;
#endif
}

uint32_t stm32f091_example_timestamp_us(void)
{
