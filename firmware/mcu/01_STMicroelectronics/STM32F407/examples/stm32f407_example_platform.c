...
static void example_delay_us(void *user, uint32_t us)
{
    (void)user;
    if (us == 0u) return;
    /* Conservative starter implementation. Replace only this callback with
     * DWT/timer based microsecond delay when tighter timing is required. */
...
{
    return stm32f407_ads1299_make_hal(hal);
}
...
void stm32f407_example_log(const char *text)
{
    if (text == NULL) return;
#if ADS1299_BOARD_USE_UART_STREAM
    (void)stm32f407_example_write((const uint8_t *)text, strlen(text));
#else
    (void)text;
#endif
}
