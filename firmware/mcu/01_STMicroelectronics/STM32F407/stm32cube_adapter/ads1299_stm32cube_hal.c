...
static int transfer_chunk(SPI_HandleTypeDef *spi,
    ...
    if (tx && rx) {
        ...
        while (offset < length) {
            ...
            if (part > ADS1299_STM32CUBE_RX_CHUNK) part = ADS1299_STM32CUBE_RX_CHUNK;
...
static int cube_spi_transfer(void *user, const uint8_t *tx, uint8_t *rx,
                             size_t length) {
    ...
    while (offset < length) {
        ...
        const uint16_t part = remaining > UINT16_MAX ? UINT16_MAX : (uint16_t)remaining;
        if (transfer_chunk(context->spi, tx ? &tx[offset] : NULL,
                           rx ? &rx[offset] : NULL, part,
                           context->spi_timeout_ms) != 0) return -1;
        offset += part;
    }
    return 0;
}
...
static void cube_pin_write(void *user, ads1299_platform_pin_t pin, int level) {
    ...
    HAL_GPIO_WritePin(context->gpio[pin].port, context->gpio[pin].pin,
