...
int main(void) {
    ...
    failures += run_test("frame_queue_fifo_wraparound_and_overflow",
                         test_frame_queue_fifo_wraparound_and_overflow);
    failures += run_test("frame_queue_argument_validation",
                         test_frame_queue_argument_validation);
    failures += run_test("frame_queue_long_running_interleaving",
                         test_frame_queue_long_running_interleaving);

    if (failures != 0) {
        printf("FAIL STM32F072 integration smoke tests (%d failures)\n", failures);
        return 1;
    }

    printf("PASS STM32F072 integration smoke tests\n");
    return 0;
}
