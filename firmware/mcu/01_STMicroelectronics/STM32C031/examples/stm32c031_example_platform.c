...
static int example_spi_transfer(void *user,
                                const uint8_t *tx,
                                uint8_t *rx,
                                size_t len)
{
    (void)user;
    if ((tx == NULL && rx == NULL) || len == 0u || len > 65535u) {
        return -1;
    }

    if (tx != NULL && rx != NULL) {
        return HAL_SPI_TransmitReceive(&ADS1299_BOARD_SPI_HANDLE,
                                       (uint8_t *)tx,
                                       rx,
                                       (uint16_t)len,
    ...
    if (tx != NULL) {
        return HAL_SPI_Transmit(&ADS1299_BOARD_SPI_HANDLE,
                                (uint8_t *)tx,
                                (uint16_t)len,
                                HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
    }

    return HAL_SPI_Receive(&ADS1299_BOARD_SPI_HANDLE,
                           rx,
                           (uint16_t)len,
                           HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
}
...
static void example_pin_write(void *user, ads1299_platform_pin_t pin, int level)
{
    ...
    switch (pin) {
    case ADS1299_PIN_CS:
        port = ADS1299_BOARD_CS_GPIO_PORT;
        gpio_pin = ADS1299_BOARD_CS_PIN;
        break;
    case ADS1299_PIN_RESET:
        port = ADS1299_BOARD_RESET_GPIO_PORT;
        gpio_pin = ADS1299_BOARD_RESET_PIN;
        break;
    case ADS1299_PIN_PWDN:
        port = ADS1299_BOARD_PWDN_GPIO_PORT;
        gpio_pin = ADS1299_BOARD_PWDN_PIN;
        break;
    case ADS1299_PIN_START:
        port = ADS1299_BOARD_START_GPIO_PORT;
        gpio_pin = ADS1299_BOARD_START_PIN;
        break;
...
static void example_delay_us(void *user, uint32_t us)
{
    (void)user;
    /*
     * HAL_Delay is deliberately conservative here. It gives the portable
     * beginner path timing margins without requiring a generated timer.
     * A board-specific microsecond timer may replace this callback later.
     */
    if (us == 0u) {
        return;
    }
...
#if ADS1299_BOARD_USE_UART_STREAM
    return HAL_UART_Transmit(&ADS1299_BOARD_UART_HANDLE,
                             (uint8_t *)data,
                             (uint16_t)len,
                             HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
#else
...
#if ADS1299_BOARD_USE_UART_STREAM
    (void)stm32c031_example_write((const uint8_t *)text, strlen(text));
#else
    (void)text;
#endif
}
...
{
    /* Millisecond HAL tick represented in the repository's microsecond field. */
