...
static int example_spi_transfer(void *user, const uint8_t *tx, uint8_t *rx, size_t len)
{
    ...
    return HAL_SPI_Receive(&hspi1, rx, (uint16_t)len,
...
static void example_pin_write(void *user, ads1299_platform_pin_t pin, int level)
{
    ...
    switch (pin) {
    case ADS1299_PIN_CS:
        port = ADS1299_H563_CS_GPIO_PORT;
        gpio_pin = ADS1299_H563_CS_GPIO_PIN;
        break;
    case ADS1299_PIN_RESET:
        port = ADS1299_H563_RESET_GPIO_PORT;
        gpio_pin = ADS1299_H563_RESET_GPIO_PIN;
        break;
    case ADS1299_PIN_PWDN:
        port = ADS1299_H563_PWDN_GPIO_PORT;
        gpio_pin = ADS1299_H563_PWDN_GPIO_PIN;
        break;
    case ADS1299_PIN_START:
        port = ADS1299_H563_START_GPIO_PORT;
        gpio_pin = ADS1299_H563_START_GPIO_PIN;
        break;
...
static int example_pin_read(void *user, ads1299_platform_pin_t pin)
{
    ...
    return HAL_GPIO_ReadPin(ADS1299_H563_DRDY_GPIO_PORT,
                            ADS1299_H563_DRDY_GPIO_PIN) == GPIO_PIN_SET ? 1 : 0;
}
...
int stm32h563_ads1299_make_hal(ads1299_platform_hal_t *hal)
{
...
/* Compatibility entry point used by the original main_ads1299.c example. */
int board_ads1299_hal(ads1299_platform_hal_t *hal)
{
    return stm32h563_ads1299_make_hal(hal);
}

int stm32h563_example_write(const uint8_t *data, size_t len)
{
    if (data == NULL || len == 0u || len > 65535u) return -1;
    return HAL_UART_Transmit(&huart3, (uint8_t *)data, (uint16_t)len,
                             HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
}

void stm32h563_example_log(const char *text)
{
    if (text == NULL) return;
    (void)stm32h563_example_write((const uint8_t *)text, strlen(text));
}

uint32_t stm32h563_example_timestamp_us(void)
{
