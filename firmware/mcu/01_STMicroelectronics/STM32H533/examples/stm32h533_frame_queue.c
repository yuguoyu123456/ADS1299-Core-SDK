...
int stm32h533_ads1299_frame_queue_push(
    ...
    uint32_t sequence)
{
    stm32h533_ads1299_frame_record_t *slot;

    if (queue == NULL || frame == NULL ||
        STM32H533_ADS1299_FRAME_QUEUE_CAPACITY == 0u) {
        return -1;
    }

    if (queue->count >= STM32H533_ADS1299_FRAME_QUEUE_CAPACITY) {
        ++queue->dropped;
        return -1;
    }
    ...
    queue->head = (queue->head + 1u) % STM32H533_ADS1299_FRAME_QUEUE_CAPACITY;
    ++queue->count;
    ...
    stm32h533_ads1299_frame_record_t *record)
{
    ...
    *record = queue->slots[queue->tail];
    queue->tail = (queue->tail + 1u) % STM32H533_ADS1299_FRAME_QUEUE_CAPACITY;
    --queue->count;
