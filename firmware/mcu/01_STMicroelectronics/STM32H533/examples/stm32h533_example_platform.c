...
static void example_pin_write(void *user, ads1299_platform_pin_t pin, int level)
{
    ...
    switch (pin) {
    case ADS1299_PIN_CS:
        port = ADS1299_H533_CS_GPIO_PORT;
        gpio_pin = ADS1299_H533_CS_GPIO_PIN;
        break;
    case ADS1299_PIN_RESET:
        port = ADS1299_H533_RESET_GPIO_PORT;
        gpio_pin = ADS1299_H533_RESET_GPIO_PIN;
        break;
    case ADS1299_PIN_PWDN:
        port = ADS1299_H533_PWDN_GPIO_PORT;
        gpio_pin = ADS1299_H533_PWDN_GPIO_PIN;
        break;
    case ADS1299_PIN_START:
        port = ADS1299_H533_START_GPIO_PORT;
        gpio_pin = ADS1299_H533_START_GPIO_PIN;
        break;
...
static int example_pin_read(void *user, ads1299_platform_pin_t pin)
{
    ...
    return HAL_GPIO_ReadPin(ADS1299_H533_DRDY_GPIO_PORT,
                            ADS1299_H533_DRDY_GPIO_PIN) == GPIO_PIN_SET ? 1 : 0;
}
...
{
    return stm32h533_ads1299_make_hal(hal);
}
...
{
    if (text == NULL) return;
    (void)stm32h533_example_write((const uint8_t *)text, strlen(text));
}
