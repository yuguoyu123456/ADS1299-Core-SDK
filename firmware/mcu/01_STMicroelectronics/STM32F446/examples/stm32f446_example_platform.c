...
int stm32f446_ads1299_make_hal(ads1299_platform_hal_t *hal)
{
...
{
    return stm32f446_ads1299_make_hal(hal);
}

int stm32f446_example_write(const uint8_t *data, size_t len)
{
    if (data == NULL || len == 0u || len > 65535u) return -1;
    return HAL_UART_Transmit(&huart2, (uint8_t *)data, (uint16_t)len,
                             HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
}

void stm32f446_example_log(const char *text)
{
    if (text == NULL) return;
    (void)stm32f446_example_write((const uint8_t *)text, strlen(text));
}

uint32_t stm32f446_example_timestamp_us(void)
{
