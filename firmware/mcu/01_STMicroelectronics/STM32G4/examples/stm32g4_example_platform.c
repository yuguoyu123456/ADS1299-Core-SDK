...
static void example_pin_write(void *user, ads1299_platform_pin_t pin, int level)
{
    ...
    switch (pin) {
    case ADS1299_PIN_CS:
        port = ADS1299_G4_CS_PORT;
        gpio_pin = ADS1299_G4_CS_PIN;
        break;
    case ADS1299_PIN_RESET:
        port = ADS1299_G4_RESET_PORT;
        gpio_pin = ADS1299_G4_RESET_PIN;
        break;
    case ADS1299_PIN_PWDN:
        port = ADS1299_G4_PWDN_PORT;
        gpio_pin = ADS1299_G4_PWDN_PIN;
        break;
    case ADS1299_PIN_START:
        port = ADS1299_G4_START_PORT;
        gpio_pin = ADS1299_G4_START_PIN;
        break;
...
static int example_pin_read(void *user, ads1299_platform_pin_t pin)
{
    ...
    return HAL_GPIO_ReadPin(ADS1299_G4_DRDY_PORT,
                            ADS1299_G4_DRDY_PIN) == GPIO_PIN_SET ? 1 : 0;
}
...
{
    return stm32g4_ads1299_make_hal(hal);
}

int stm32g4_example_write(const uint8_t *data, size_t len)
{
    if (data == NULL || len == 0u || len > 65535u) return -1;
    return HAL_UART_Transmit(&hlpuart1, (uint8_t *)data, (uint16_t)len,
                             HAL_MAX_DELAY) == HAL_OK ? 0 : -1;
}
...
{
    if (text == NULL) return;
    (void)stm32g4_example_write((const uint8_t *)text, strlen(text));
}
