...
This directory adapts STM32F429 board/HAL services to the shared portable `ads1299_port_t` contract. ADS1299 register policy, profiles, frame parsing, packet formatting and multi-device behavior belong in `firmware/core_driver/ads1299/`, not in this folder.
...
```text
PASS STM32F429 ads1299_port host self-test
```
...
A new STM32F429 board should provide the concrete STM32 HAL callbacks and pin/peripheral selection in the model's board/config layer, then populate `ads1299_platform_hal_t`. Do not edit shared ADS1299 core files for normal board bring-up.
...
- Physical STM32F429 + ADS1299 board validation: NOT CLAIMED.
