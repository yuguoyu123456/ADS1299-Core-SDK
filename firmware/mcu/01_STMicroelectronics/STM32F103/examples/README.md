...
Use the `NUCLEO-F103RB` reference configuration documented in `../board/README.md` and `../board/board_config.h`.
...
- `stm32f103_example_platform.c/.h`: STM32Cube HAL binding for SPI, GPIO, DRDY, delays, UART logging/streaming and timestamping.
- `stm32f103_beginner_demo.c/.h`: complete beginner sequence: probe ID -> internal test -> input-short -> 250-SPS EEG -> canonical packet stream.
- `main_ads1299.c`: older minimal example retained for compatibility. Its `board_ads1299_hal()` dependency is now provided by `stm32f103_example_platform.c`.

## CubeMX / CubeIDE integration

Create an official STM32CubeIDE project for NUCLEO-F103RB and configure:
...
(void)stm32f103_ads1299_beginner_demo(1000u);
```
...
```text
ADS1299 STM32F103 beginner demo
OK ID ADS1299-8
RUN internal-test
...
Changing to another STM32F103 board should normally require only CubeMX peripheral/pin changes and the board/config binding. Do not edit `ads1299.c`, `ads1299_regs.h` or shared model logic for a normal board change.

## Validation boundary

This folder provides a concrete integration recipe and source implementation. It does **not** by itself prove that an STM32CubeIDE project has compiled successfully or that NUCLEO-F103RB + ADS1299 hardware has been exercised. Promote status only when corresponding build or board evidence exists.
