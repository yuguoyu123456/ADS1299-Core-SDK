...
Use the `NUCLEO-F303RE` reference configuration documented in `../board/README.md` and `../board/board_config.h`.
...
- `stm32f303_example_platform.c/.h`: STM32Cube HAL binding for SPI, GPIO, DRDY, delays, UART logging/streaming and timestamping.
- `stm32f303_beginner_demo.c/.h`: complete beginner sequence: probe ID -> internal test -> input-short -> 250-SPS EEG -> canonical packet stream.
- `main_ads1299.c`: older minimal example retained for compatibility. Its `board_ads1299_hal()` dependency is now provided by `stm32f303_example_platform.c`.

## CubeMX / CubeIDE integration

Create an official STM32CubeIDE project for NUCLEO-F303RE and configure:
...
4. USART2 when `ADS1299_BOARD_USE_UART_STREAM` is enabled. The reference board config uses `huart2`.
...
```c
#include "stm32f303_beginner_demo.h"

/* Stream 1000 EEG packets and return. Use 0 for continuous streaming. */
(void)stm32f303_ads1299_beginner_demo(1000u);
```
...
```text
ADS1299 STM32F303 beginner demo
OK ID ADS1299-8
RUN internal-test
...
Changing to another STM32F303 board should normally require only CubeMX peripheral/pin changes and the board/config binding. Do not edit `ads1299.c`, `ads1299_regs.h` or shared model logic for a normal board change.

## Timing note

The starter HAL binding rounds microsecond delays up to the STM32 HAL millisecond tick. This is intentionally simple and bounded for first bring-up. Applications that require precise sub-millisecond timing should replace only the platform delay callback with a verified timer/DWT implementation; ADS1299 register logic still remains in the shared core.

## Validation boundary

This folder provides a concrete integration recipe and source implementation. It does **not** by itself prove that an STM32CubeIDE project has compiled successfully or that NUCLEO-F303RE + ADS1299 hardware has been exercised. Promote status only when corresponding build or board evidence exists.
