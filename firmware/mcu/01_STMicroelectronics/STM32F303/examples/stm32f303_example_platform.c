...
static void example_delay_us(void *user, uint32_t us)
{
    (void)user;
    if (us == 0u) return;
    /* Beginner/reference implementation: millisecond HAL tick rounding.
     * Replace only this platform callback with a hardware timer/DWT based
     * implementation when sub-millisecond timing precision is required. */
...
/* Compatibility entry point used by the older minimal example. */
int board_ads1299_hal(ads1299_platform_hal_t *hal)
{
    return stm32f303_ads1299_make_hal(hal);
}
...
void stm32f303_example_log(const char *text)
{
    if (text == NULL) return;
#if ADS1299_BOARD_USE_UART_STREAM
    (void)stm32f303_example_write((const uint8_t *)text, strlen(text));
#else
    (void)text;
#endif
}
