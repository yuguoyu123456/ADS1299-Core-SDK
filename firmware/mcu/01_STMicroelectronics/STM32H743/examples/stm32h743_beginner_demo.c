...
#define STM32H743_DEMO_DRDY_TIMEOUT_US 1000000u
#define STM32H743_DEMO_DRDY_POLL_US       1000u
#define STM32H743_DEMO_TEST_FRAMES            8u

extern int board_ads1299_hal(ads1299_platform_hal_t *hal);

static void demo_log(const stm32h743_ads1299_demo_io_t *io, const char *message)
{
...
static int wait_and_read(const stm32h743_ads1299_demo_io_t *io,
                         ads1299_t *dev,
                         ads1299_frame_t *frame)
{
    ...
    st = ads1299_wait_drdy(dev,
                           STM32H743_DEMO_DRDY_TIMEOUT_US,
                           STM32H743_DEMO_DRDY_POLL_US);
    if (st != ADS1299_OK) {
...
static int capture_frames(const stm32h743_ads1299_demo_io_t *io,
                          ads1299_t *dev,
                          uint32_t count)
{
    ads1299_frame_t frame;
...
static int probe_device(const stm32h743_ads1299_demo_io_t *io, ads1299_t *dev)
{
...
static int run_internal_test(const stm32h743_ads1299_demo_io_t *io,
                             ads1299_t *dev)
{
    ...
    if (capture_frames(io, dev, STM32H743_DEMO_TEST_FRAMES) != 0) {
...
static int run_input_short(const stm32h743_ads1299_demo_io_t *io,
                           ads1299_t *dev)
{
...
static int configure_eeg_250sps(const stm32h743_ads1299_demo_io_t *io,
                                ads1299_t *dev)
{
    uint8_t ch;
...
static int stream_packets(const stm32h743_ads1299_demo_io_t *io,
                          ads1299_t *dev,
                          uint32_t stream_frames)
{
    ...
    if (io == NULL || io->write == NULL || io->timestamp_us == NULL) {
        demo_log(io, "ERR stream callbacks/write timestamp_us required\r\n");
        return -1;
    }
                                    ...
                                    uint32_t stream_frames)
{
    ...
    demo_log(io, "ADS1299 STM32H743 beginner demo\r\n");
