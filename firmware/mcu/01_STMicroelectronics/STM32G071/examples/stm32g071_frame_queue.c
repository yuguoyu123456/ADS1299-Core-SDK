...
int stm32g071_ads1299_frame_queue_push(stm32g071_ads1299_frame_queue_t *queue,
                                       ...
                                       uint32_t sequence)
{
    stm32g071_ads1299_frame_record_t *slot;
    if (queue == NULL || frame == NULL) return -1;
    if (queue->count >= STM32G071_ADS1299_FRAME_QUEUE_CAPACITY) {
        ++queue->dropped;
        return -1;
    }
    ...
    queue->head = (queue->head + 1u) % STM32G071_ADS1299_FRAME_QUEUE_CAPACITY;
    ++queue->count;
                                      ...
                                      stm32g071_ads1299_frame_record_t *record)
{
    ...
    *record = queue->slots[queue->tail];
    queue->tail = (queue->tail + 1u) % STM32G071_ADS1299_FRAME_QUEUE_CAPACITY;
    --queue->count;
    return 0;
}
