...
int stm32f767_ads1299_frame_queue_push(stm32f767_ads1299_frame_queue_t *queue,
                                       ...
                                       uint32_t sequence)
{
    stm32f767_ads1299_frame_record_t *slot;

    if (queue == NULL || frame == NULL || STM32F767_ADS1299_FRAME_QUEUE_CAPACITY == 0u) {
        return -1;
    }
    if (queue->count >= STM32F767_ADS1299_FRAME_QUEUE_CAPACITY) {
        ++queue->dropped;
        return -1;
    }
    ...
    queue->head = (queue->head + 1u) % STM32F767_ADS1299_FRAME_QUEUE_CAPACITY;
    ++queue->count;
                                      ...
                                      stm32f767_ads1299_frame_record_t *record)
{
    if (queue == NULL || record == NULL || queue->count == 0u ||
        STM32F767_ADS1299_FRAME_QUEUE_CAPACITY == 0u) {
        return -1;
    }

    *record = queue->slots[queue->tail];
    queue->tail = (queue->tail + 1u) % STM32F767_ADS1299_FRAME_QUEUE_CAPACITY;
    --queue->count;
    return 0;
}
