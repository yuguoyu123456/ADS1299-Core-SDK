# Integration with GD32F4xx Firmware Library

1. Install the official `GD32F4xx Firmware Library` environment; do not copy the vendor SDK into this repository.
2. Create or open a vendor project for `GD32F450ZKT6` / `GD32F450Z-EVAL`.
...
callback adapter and ADS1299-independent tests. Pin `pinned PlatformIO package reference` in the
