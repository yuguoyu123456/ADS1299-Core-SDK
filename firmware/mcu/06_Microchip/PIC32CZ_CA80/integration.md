# Integration with MPLAB Harmony / device packs; AVR uses its own toolchain

1. Install the official `MPLAB Harmony / device packs; AVR uses its own toolchain` environment; do not copy the vendor SDK into this repository.
2. Create or open a vendor project for `PIC32CZ_CA80` / `Select an official PIC32CZ_CA80 evaluation board`.
