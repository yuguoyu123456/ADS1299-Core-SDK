#include "ads1299_mcxw236_hal.h"

#include <stddef.h>

#define ADS1299_MCXW236_MAX_BRINGUP_SPI_HZ 4000000u

static int gpio_valid(const ads1299_mcxw236_gpio_t *p)
{
    return p != NULL && p->gpio != NULL && p->pin < 32u;
}

static int gpio_same(const ads1299_mcxw236_gpio_t *a,
                     const ads1299_mcxw236_gpio_t *b)
{
    return a->gpio == b->gpio && a->port == b->port && a->pin == b->pin;
}

static int adapter_spi_transfer(void *user, const uint8_t *tx, uint8_t *rx, size_t len)
{
    ads1299_mcxw236_hal_context_t *ctx = (ads1299_mcxw236_hal_context_t *)user;
...
static ads1299_mcxw236_gpio_t *pin_for(ads1299_mcxw236_hal_context_t *ctx,
...
{
    ads1299_mcxw236_hal_context_t *ctx = (ads1299_mcxw236_hal_context_t *)user;
    ads1299_mcxw236_gpio_t *p = ctx != NULL ? pin_for(ctx, pin) : NULL;
    if (gpio_valid(p)) {
        GPIO_PinWrite(p->gpio, p->port, p->pin, level != 0 ? 1u : 0u);
    }
}
...
static int adapter_pin_read(void *user, ads1299_platform_pin_t pin)
{
    ads1299_mcxw236_hal_context_t *ctx = (ads1299_mcxw236_hal_context_t *)user;
    ads1299_mcxw236_gpio_t *p = ctx != NULL ? pin_for(ctx, pin) : NULL;
    return gpio_valid(p) ? (int)GPIO_PinRead(p->gpio, p->port, p->pin) : -1;
}
...
int ads1299_mcxw236_hal_init(ads1299_mcxw236_hal_context_t *ctx)
{
    lpspi_master_config_t config;
    const ads1299_mcxw236_gpio_t *pins[5];
    size_t i;
    size_t j;

    if (ctx == NULL || ctx->lpspi == NULL || ctx->source_clock_hz == 0u ||
        ctx->baud_hz == 0u || ctx->baud_hz > ADS1299_MCXW236_MAX_BRINGUP_SPI_HZ ||
        ctx->delay_us == NULL) {
        return -1;
    }
...
ads1299_platform_hal_t ads1299_mcxw236_make_hal(ads1299_mcxw236_hal_context_t *ctx)
{
