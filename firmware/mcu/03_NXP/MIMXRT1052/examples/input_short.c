/* MIMXRT1052 progressive bring-up: ADS1299 input-short validation.
 *
 * Uses the shared ADS1299 named configuration; no platform register literals.
 * Exit codes intentionally separate board/port, reset/state, profile, DRDY,
 * and frame-transfer failures for first-board bring-up.
 */
#include "ads1299.h"
...
int main(void)
{
    ...
    /* Shared-core validation profile: all channel inputs shorted internally. */
    ...
    for (;;) {
        ...
        if (ready == 0) {
            if (ads1299_read_frame_continuous(&dev, &frame) != ADS1299_OK)
                return 50;
            /* Inspect/log frame channels in the consuming board application.
             * Shorted-input samples should remain near converter baseline.
             */
        }
    }
}
