/* MIMXRT1042 bounded acquisition example.
...
/* Call this from the foreground transport task/loop. Encode/send the returned
 * frame outside the DRDY path; do not block acquisition on transport I/O. */
...
int main(void) {
    ...
    for (;;) {
        ...
        /* Drain with ads1299_stream_try_pop() from a foreground task/transport
         * service. Keep UART/USB/network blocking work out of this path. */
    }
}
