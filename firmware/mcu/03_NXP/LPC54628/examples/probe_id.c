/* LPC54628 progressive bring-up: ADS1299 probe / ID validation.
...
int main(void)
{
    ...
    if (ads1299_mcu_port_init(&mcu, &hal) != 0) return 11; /* port/HAL */
    ...
    if (ads1299_read_device_id(&dev, &id) != ADS1299_OK) return 30; /* SPI/ID */
    if (!id.is_ads1299_family) return 31;

    /* Attach a debugger here: id now contains the decoded ADS1299 identity. */
    return 0;
}
