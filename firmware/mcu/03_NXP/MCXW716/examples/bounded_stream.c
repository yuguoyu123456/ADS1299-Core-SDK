/* MCXW716 bounded acquisition example.
 * Producer: ADS1299 DRDY polling + frame read. Consumer/transport is deliberately
 * separate so BLE/802.15.4/network/UART/USB work never blocks acquisition. */
#include "ads1299.h"
...
int main(void) {
    ...
    for (;;) {
         ...
         * Do not perform BLE/802.15.4/network transmission here. If producer
         * and consumer move to different ISR/RTOS contexts, add the platform's
         * appropriate synchronization primitive around queue ownership. */
    }
}
