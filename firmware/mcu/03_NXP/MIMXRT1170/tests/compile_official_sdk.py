...
parser.add_argument('--link-reference-ram', action='store_true',
                    help='Also link the EVK-derived RAM-debug image using external SDK startup/linker')
...
includes = [args.sdk / p for p in ('devices/MIMXRT1176',
    'devices/MIMXRT1176/drivers', 'drivers/common', 'drivers/igpio', 'drivers/lpspi')]
required = [includes[0] / 'MIMXRT1176_cm7.h', includes[2] / 'fsl_common.h',
...
sdk_includes = includes + [args.cmsis]
...
sources += [target / 'examples/main_ads1299.c', target / 'board/evk_reference.c']
...
with tempfile.TemporaryDirectory(prefix='.official-sdk-', dir=target / 'tests') as temporary:
    for index, source in enumerate(sources):
        subprocess.run([args.cc, '-std=c11', '-Wall', '-Wextra', '-Werror',
            '-mcpu=cortex-m7', '-mthumb', '-DCPU_MIMXRT1176DVMAA_cm7',
    ...
    if args.cmake:
        ...
        if args.link_reference_ram:
            elf = build / 'ads1299_evk_reference.elf'
...
print(json.dumps({'result': 'official-header object compilation passed',
    ...
    'evk_reference_ram_link': 'passed' if args.link_reference_ram else 'not run',
    'image_evidence': image_evidence,
    'compiler': version, 'translation_units': len(sources),
    'scope': 'EVK-derived reference only; no hardware execution or EVKB wiring validation',
