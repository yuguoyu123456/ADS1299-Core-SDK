#include "ads1299_mimxrt1011_hal.h"

#include <stddef.h>

#define ADS1299_MIMXRT1011_MAX_BRINGUP_SPI_HZ 4000000u

static int gpio_valid(const ads1299_mimxrt1011_gpio_t *p)
{
    return p != NULL && p->gpio != NULL && p->pin < 32u;
}

static int gpio_same(const ads1299_mimxrt1011_gpio_t *a,
                     const ads1299_mimxrt1011_gpio_t *b)
{
...
{
    ads1299_mimxrt1011_hal_context_t *ctx = (ads1299_mimxrt1011_hal_context_t *)user;
...
static ads1299_mimxrt1011_gpio_t *pin_for(ads1299_mimxrt1011_hal_context_t *ctx,
...
{
    ads1299_mimxrt1011_hal_context_t *ctx = (ads1299_mimxrt1011_hal_context_t *)user;
    ads1299_mimxrt1011_gpio_t *p = ctx != NULL ? pin_for(ctx, pin) : NULL;
    if (gpio_valid(p)) {
...
int ads1299_mimxrt1011_hal_init(ads1299_mimxrt1011_hal_context_t *ctx)
{
    lpspi_master_config_t config;
    const ads1299_mimxrt1011_gpio_t *pins[5];
    size_t i;
    size_t j;

    if (ctx == NULL || ctx->lpspi == NULL || ctx->source_clock_hz == 0u ||
        ctx->baud_hz == 0u || ctx->baud_hz > ADS1299_MIMXRT1011_MAX_BRINGUP_SPI_HZ ||
        ctx->delay_us == NULL) {
        return -1;
    }
...
ads1299_platform_hal_t ads1299_mimxrt1011_make_hal(ads1299_mimxrt1011_hal_context_t *ctx)
{
